...
// and left as it was found. Every paragraph below is a bug somebody already
// paid for, and the shape of the fix is not obvious from the outside — a
// rewrite here would rediscover them one at a time.
//
...
export function useTranscriptScroll({
  ...
  const scrollToBottom = useCallback((behavior: ScrollBehavior = "instant") => {
    ...
    // ADDED HERE, not in the original. Some browsers ignore
    // `behavior: "smooth"` on a scroller outright — measured in the preview
    // browser, where the same element moves 2351px for "instant" and not one
    // pixel for "smooth". The failure is silent and lands on the one control
    // whose entire job is to move: a reader presses "jump to newest" and
    // nothing happens. So if nothing has moved shortly after, it is done the
    // way that always works.
    if (behavior !== "smooth") return;
    window.setTimeout(() => {
      const node = containerRef.current;
      if (node && Math.abs(node.scrollTop - before) < 1) write(node.scrollHeight);
    }, 80);
  }, [setFollowing, write]);

...
import { beingDataUri } from "@/site/identity/being-svg";
import {
    buildVCard,
    contactCardFilename,
    contactCardParams,
...
import { SHARE_NAME_PARAM } from "@/site/share/share-link";
import {
    identityVariantSeed,
    parseResourceIcon,
} from "@/site/utils/resource-icon-value";
...
async function generatedPhoto(seed: string): Promise<VCardPhoto | null> {
    try {
        ...
        return {
            data: base64(new Uint8Array(await image.arrayBuffer())),
            type: "PNG",
        };
    } catch {
...
export async function GET(
    ...
    const page = new URL(
        `/s/contact/${segments.map(encodeURIComponent).join("/")}`,
        origin,
    );

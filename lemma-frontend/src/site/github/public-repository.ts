...
function isDecorativeReadmeImage(value: string): boolean {
    ...
    const isOwnActions =
        host === "github.com" &&
        path.startsWith("/lemma-work/lemma-platform/actions/");
    return (
        isBadgeHost ||
        isOwnActions ||
        path.includes("/badge") ||
...
function readmeImages(markdown: string): ReadmeImage[] {
    ...
    for (const match of markdown.matchAll(
        ...
        if (match[1])
            images.push({
                src: match[1].trim(),
                maxWidth: declaredImageWidth(match[0]),
            });
    }
    for (const match of markdown.matchAll(
        /!\[[^\]]*]\(([^)\s]+)(?:\s+["'][^"']*["'])?\)/gi,
...
export function extractReadmePresentation(
    ...
    const cover = readmeImages(markdown).find(
        (image) => !isDecorativeReadmeImage(image.src),
    );
    ...
    const firstMarkdownHeading =
        maskFencedCode(bodyWithPreamble).search(/^#{1,6}\s+/m);
    const preambleEnd =
        firstMarkdownHeading >= 0
            ? firstMarkdownHeading
            : bodyWithPreamble.length;

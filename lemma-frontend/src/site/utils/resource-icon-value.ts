...
import { identityGenes } from "@/site/identity/seeded-identity";
 ...
 */
export function parseResourceIcon(
    value?: string | null,
): ResourceIconValue | null {
 ...
 */
export function identityHueClass(
...
export function distinctIdentityVariants(
    ...
    for (
        let variant = 0;
        variant < 800 && chosen.length < count;
        variant += 1
    ) {

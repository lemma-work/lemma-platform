...
const BUILDS: {
    ...
    },
    {
        ...
        patterns: [
            /^Lemma_.+_x64-online-setup\.exe$/,
            /^Lemma_.+_x64-setup\.exe$/,
        ],
    },
];
 ...
 */
export function selectDesktopBuilds(
    ...
    for (const { platform, label, requirement, patterns } of BUILDS) {
        ...
        builds.push({
            ...
            size:
                typeof match.size === "number" && match.size > 0
                    ? match.size
                    : null,
        });
    }
...
export async function fetchLatestDesktopRelease(): Promise<DesktopRelease> {
    ...
    try {
        ...
        const assets = Array.isArray(release.assets)
            ? (release.assets as ReleaseAsset[])
            : [];
        return {

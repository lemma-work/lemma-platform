...
export interface IdentityGenes {
     ...
     * roster blinks in lockstep — which does not read as a group of agents,
 ...
 */
const POD_ARRANGEMENTS: ReadonlyArray<
...
export const STATE_LOOKS: Record<IdentityState, StateLook> = {
    idle: {
        eye: { kind: "open", pupilX: 0, pupilY: 0, lid: 0 },
        pip: null,
        opacity: 1,
    },
    thinking: {
        eye: { kind: "open", pupilX: -0.34, pupilY: -0.36, lid: 0.34 },
        pip: "var(--state-info)",
        opacity: 1,
    },
    running: {
        eye: { kind: "open", pupilX: 0, pupilY: 0, lid: 0 },
        pip: "var(--state-success)",
        opacity: 1,
    },
    waiting: {
        eye: { kind: "wide", pupilX: 0, pupilY: 0.2, lid: 0 },
        pip: "var(--state-warning)",
        opacity: 1,
    },
    failed: {
        eye: { kind: "closed", pupilX: 0, pupilY: 0, lid: 0 },
        pip: "var(--state-error)",
        opacity: 1,
    },
    asleep: {
        eye: { kind: "line", pupilX: 0, pupilY: 0, lid: 0 },
        pip: null,
        opacity: 0.4,
    },
};

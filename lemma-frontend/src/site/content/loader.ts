...
const YAML_ENGINE = {
    parse: (input: string) =>
        loadYaml(input, { schema: CORE_SCHEMA }) as object,
    stringify: () => {
...
function parseIsoDate(value: unknown, field: string, source: string): string {
    if (typeof value !== "string" || !/^\d{4}-\d{2}-\d{2}$/.test(value)) {
        throw new Error(
            `${source}: "${field}" must be a YYYY-MM-DD string, got ${JSON.stringify(value)}`,
        );
    }
...
function requireString(value: unknown, field: string, source: string): string {
    if (typeof value !== "string" || value.trim() === "") {
        throw new Error(
            `${source}: "${field}" is required and must be a non-empty string`,
        );
    }
    return value.trim();
}
...
function parseFrontmatter(
    ...
    const updated =
        data.updated === undefined
            ? undefined
            : parseIsoDate(data.updated, "updated", source);
    if (updated && updated < published) {
        throw new Error(
            `${source}: "updated" (${updated}) is before "published" (${published})`,
        );
    }
    ...
    return {
        ...
        author:
            data.author === undefined
                ? undefined
                : requireString(data.author, "author", source),
        tags: tags as string[],
        pod: data.pod,
        cover: data.cover,
...
function readDoc(collection: ContentCollection, fileName: string): ContentDoc {
    ...
    const raw = fs.readFileSync(
        path.join(collectionDir(collection), fileName),
        "utf8",
    );
    ...
    return fs
        ...
        .sort((a, b) => {
            if (a.frontmatter.published !== b.frontmatter.published) {
                return a.frontmatter.published < b.frontmatter.published
                    ? 1
                    : -1;
            }
...
export function collectionLastModified(
    collection: ContentCollection,
): string | null {

...
export function getPublicTemplateBySlug(
...
export function templateGitHubSource(template: PublicTemplate): GitHubSource {
    ...
    if (source.hostname.toLowerCase() !== "github.com") {
        throw new Error(
            `Template "${template.slug}" must use a github.com source.`,
        );
    }
    ...
    if (!owner || !repo) {
        throw new Error(
            `Template "${template.slug}" has an invalid GitHub source.`,
        );
    }
    return { owner, repo };
}
 ...
 */
export function findPublicTemplateBySource(
    owner: string,
    repo: string,
): PublicTemplate | null {
    ...
    return (
        PUBLIC_TEMPLATES.find((template) => {
            ...
            return (
                `${source.owner.toLowerCase()}/${source.repo.toLowerCase()}` ===
                wanted
            );
        }) ?? null
    );

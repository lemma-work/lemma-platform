 ...
 * 7 back would cost the roster twenty buckets (160 → 140) to identify one
 * creature, which is a bad trade in a system whose whole problem is collisions.
 * Appending costs nothing — the generator still rolls 0–7, so every agent's
 * face is bit-for-bit unchanged by the reservation.
 */
...
export interface IdentityGenes {
     ...
     * roster blinks in lockstep — which does not read as a pod full of agents,
 ...
 *
 * A seeded face is wrong twice over here: it introduces a stranger on the one
 * row meant to be the responder you already know, and it is a different
 * stranger in every pod. The Lemma trademark on a tinted tile is wrong
 * differently — it draws the pod's most capable agent in the treatment
 * reserved for *inert* things, so the being with the most agency in the system
 * is the one that cannot open its eyes, carry a state pip, or move, and the
 * recents list needs a special rule to work around it.
 *
 ...
 * marks follow, and for the same measured reason: two hues at full strength
 * clash in every combination this palette can produce.
 */
 ...
 * below about 32px, and the pod switcher never gets that much space.
 *
 ...
 *
 * So the ground, the light field and the solid field are all the same hue at
 * increasing strength, and the overlap is darker still. A monochrome ramp
 * cannot clash with itself, so every pod is harmonious by construction, and
 * the variety a second hue would buy comes from the arrangement and the form
 * pair instead — which costs nothing to harmony.
 */

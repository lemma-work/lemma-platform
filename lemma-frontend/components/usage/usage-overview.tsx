...
import { costSourceLabel } from "./usage-composition";
import { UsageTokenBreakdown } from "./usage-token-breakdown";
...
export function UsageOverview({
  ...
  const summary = useUsageSummary(organizationId, filters, {
    enabled: scope !== "pod" || Boolean(podId),
  });
  ...
  const recent = useRecentUsage(organizationId, filters, {
    enabled: scope !== "pod" || Boolean(podId),
  });
      ...
      {isLoading ? (
        ...
        <>
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(21rem,0.75fr)]">
            ...
            <UsageTokenBreakdown summary={summary.data} />
          </div>
...
function UsageMetricStrip({
  ...
  const remaining =
    scope === "organization"
      ? limitScope?.remaining_usd
      : userLimitScope?.remaining_usd;
  const resetAt =
    scope === "organization" ? limitScope?.reset_at : userLimitScope?.reset_at;
  ...
  return (
    <ResourceMetricStrip className="lemma-index-tabs-left p-0">
      <ResourceMetric
        label="Cost"
        value={formatCurrency(summary?.system_cost_usd)}
      />
      <ResourceMetric
        label="Tokens"
      ...
      <ResourceMetric
        label={
          scope === "organization" ? "Org monthly left" : "Your weekly left"
        }
      ...
      />
      {resetAt ? (
        <ResourceMetric label="Resets" value={formatDate(resetAt)} />
      ) : null}
    </ResourceMetricStrip>
  );
}
      ...
      <div className="mb-4 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">
            Daily spend buckets from the selected window.
          </p>
        </div>
      ...
      {buckets.length === 0 ? (
        ...
        <div className="space-y-3">
          {buckets.slice(-12).map((bucket) => {
            const width =
              maxCost > 0
                ? Math.max(4, (bucket.system_cost_usd / maxCost) * 100)
                : 4;
            return (
              <div
                key={`${bucket.bucket}-${bucket.group || "all"}`}
                className="grid gap-2 sm:grid-cols-[6.5rem_minmax(0,1fr)_5.5rem] sm:items-center"
              >
  ...
  return (
    <section className="surface-panel p-4">
      <div className="mb-4">
        ...
        <p className="mt-1 text-xs leading-5 text-[var(--text-secondary)]">
          Grouped totals returned by the billing API.
        </p>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <div className="space-y-2">
          {rows.slice(0, 5).map((row) => (
            <div
              key={`${title}-${row.label}`}
              className="surface-panel-muted px-3 py-2"
            >
          ...
          {records.map((record) => (
            ...
            >
              ...
              <span className="hidden shrink-0 text-xs text-[var(--text-secondary)] sm:inline">
                {formatRecordTokens(record)}
              </span>
              <CostSourceBadge source={record.cost_source} />
...
/** One row's tokens, split the way the price was.
 *
 * `in` is the inclusive total and the cached figure is a part of it, so the two
 * are written as one phrase rather than two counts a reader would add up.
 */
function formatRecordTokens(record: UsageRecord): string {
  const parts = [`${formatCompact(record.input_tokens)} in`];
  if (record.cached_input_tokens) {
    parts.push(`${formatCompact(record.cached_input_tokens)} cached`);
  }
  parts.push(`${formatCompact(record.output_tokens)} out`);
  return parts.join(" · ");
}

/** Where this row's price came from, when the answer is worth acting on.
 *
 * An estimate and a configured rate are both a number in the same column, and
 * an operator about to invoice somebody needs to be able to tell them apart.
 * `REGISTERED` is the unremarkable case and stays unlabelled — a badge on every
 * row is a badge nobody reads.
 */
function CostSourceBadge({ source }: { source?: string | null }) {
  const described = costSourceLabel(source);
  if (!described || described.label === "Rated") return null;
  return (
    <Badge
      variant="outline"
      className="hidden shrink-0 md:inline-flex"
      ...
      <div>
        ...
        <p className="mt-1 text-[var(--text-secondary)]">
          {error instanceof Error
            ? error.message
            : "The usage API returned an error."}
        </p>
      </div>
    </div>
  );
...
function flattenTotals(
  ...
  return Object.entries(source || {})
    .map(([label, values]) => {
      ...
      const costEntry = entries.find(([key]) =>
        key.toLowerCase().includes("cost"),
      );
      const tokenEntry = entries.find(([key]) =>
      ...
      return {
        ...
        primary: primaryEntry
          ? formatMetric(primaryEntry[0], primaryEntry[1])
          : "No value",
        detail: entries

...
describe('mapping a click onto the page', () => {
    const frame = { pageWidth: 1280, pageHeight: 800 };
    ...
    it('accounts for the letterbox when the pane is a different shape', () => {
        ...
        const topOfImage = toPagePoint(rect, frame, { clientX: 0, clientY: 150 });
        expect(topOfImage.y).toBe(0);

        const bottomOfImage = toPagePoint(rect, frame, { clientX: 0, clientY: 650 });
        expect(bottomOfImage.y).toBe(800);
    });

    it('maps the centre to the centre', () => {
        const rect = { left: 0, top: 0, width: 800, height: 800 };
        const point = toPagePoint(rect, frame, { clientX: 400, clientY: 400 });
        expect(point).toEqual({ x: 640, y: 400 });
    });

    it('is exact when the shapes already agree', () => {
        const rect = { left: 0, top: 0, width: 640, height: 400 };
        expect(toPagePoint(rect, frame, { clientX: 320, clientY: 200 })).toEqual({
            x: 640,
            y: 400,
        });
    });
    ...
    it('accounts for where the pane sits on the screen', () => {
        ...
        expect(toPagePoint(rect, frame, { clientX: 100, clientY: 50 })).toEqual({
            x: 0,
            y: 0,
        });
    });
    ...
    it('never reports a point outside the page', () => {
        const rect = { left: 0, top: 0, width: 640, height: 400 };
        const point = toPagePoint(rect, frame, { clientX: 9999, clientY: 9999 });
        expect(point.x).toBeLessThanOrEqual(frame.pageWidth);
        expect(point.y).toBeLessThanOrEqual(frame.pageHeight);
    });

    it('survives being asked before the first frame', () => {
        const rect = { left: 0, top: 0, width: 0, height: 0 };
        expect(toPagePoint(rect, frame, { clientX: 10, clientY: 10 })).toEqual({
            x: 0,
            y: 0,
        });
    });

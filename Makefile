...
_init-backend-env:
	@if [ ! -f $(BACKEND_DIR)/.env ]; then \
		...
		{ \
			...
			echo "FUNCTION_RUNTIME_GATEWAY_URL=$(DEV_SANDBOX_BACKEND_URL)"; \
...
_ensure-backend-env-keys:
	...
	if [ -z "$$missing" ]; then \
		...
		append FUNCTION_RUNTIME_GATEWAY_URL '$(DEV_SANDBOX_BACKEND_URL)'; \

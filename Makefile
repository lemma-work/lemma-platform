...
# Apps are served by the backend at <slug>.<this>. sslip.io is wildcard DNS
# (anything.127-0-0-1.sslip.io -> 127.0.0.1), so app subdomains resolve locally
# where plain `localhost` can't carry a subdomain label.
DEV_APP_BASE_DOMAIN   := 127-0-0-1.sslip.io:$(DEV_BACKEND_PORT)
...
_init-backend-env:
	@if [ ! -f $(BACKEND_DIR)/.env ]; then \
		...
		{ \
			...
			echo "# Scheduler routes are served by the backend itself."; \
			echo "SCHEDULER_API_URL=$(DEV_BACKEND_URL)"; \
			echo "# Apps are served by host at <slug>.<APP_BASE_DOMAIN>."; \
...
_ensure-backend-env-keys:
	@set -e; missing=""; \
	for k in API_URL SCHEDULER_API_URL APP_BASE_DOMAIN FRONTEND_URL AUTH_FRONTEND_URL SUPERTOKENS_CORE_URL DATABASE_URL REDIS_URL CORS_ORIGINS CORS_ORIGIN_REGEX AGENTBOX_API_URL AGENTBOX_API_KEY; do \

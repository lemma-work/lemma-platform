...
| `SNOOZE` | **Always on.**  suspend the current turn and resume it later after a delay (`snooze`), capped at 24h. Every wake replays the whole conversation, so it is for work with a genuine gap in the middle (a build, a batch job). Prefer ending the turn over snoozing on a `message_user` answer — an ending turn costs no replay, and the backend starts a fresh one when the last answer lands. Snoozing anyway is safe, not broken: the answer resolves the pause and wakes the run early with `woke_because='ANSWERED'` |
...
`CONNECTORS`, `SUBAGENTS`, `MESSAGING`, `SNOOZE` and `BROWSER` are not in the model's

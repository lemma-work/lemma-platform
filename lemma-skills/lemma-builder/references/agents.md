...
| `MESSAGING` | reach a *pod member who is not in this conversation* on whichever surface they last used, with a copy always landing in their Lemma inbox (`message_user` / `check_messages`). Opt-in, and a real capability to hand out: the recipient sees the pod's bot and extends it the trust they extend to Lemma, so every message names both the agent and the human whose authority the run carries. It does **not** pause the run — the reply is handled by the recipient's own agent, under their permissions, guided by the `background_instruction`. Reaching anyone other than the run's own owner additionally needs the surface's `send_policy.audience` set to `POD_MEMBERS` |

...
class _PythonAllocationFailureClient(_CanonicalClient):
    ...
    async def create_python_session(self, *_args: Any, **_kwargs: Any) -> None:
        ...
        raise SandboxRejected("retry after allocation replacement")


def _session(client: _CanonicalClient) -> SandboxWorkspaceSession:
...
class _CapacityThenSucceedClient:
    ...
    async def start_process(self, *_args: Any, **kwargs: Any) -> None:
        ...
        if self.remaining_rejections > 0:
            ...
            # Retryable: the caller is expected to wait and try again, which
            # is what `SandboxUnavailable` means and `SandboxRejected` does not.
            raise SandboxUnavailable(
                "process routing capacity is full", retry_after_ms=1
            )
...
async def test_polling_a_process_again_does_not_replay_delivered_output() -> None:
    ...
    def build() -> SandboxWorkspaceSession:
        return SandboxWorkspaceSession(
            client=client,  # type: ignore[arg-type]
...
async def test_unhealthy_python_allocation_fails_once_without_retry_storm() -> None:
    ...
    assert result.error_in_exec["ename"] == "SandboxRejected"

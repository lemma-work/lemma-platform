...
Nothing here starts a sandbox -- `_provisioning_env` is the pure function that
...
from app.modules.workspace.services.sandbox_service import _provisioning_env
...
def test_a_function_sandbox_never_gets_a_proxy(monkeypatch) -> None:
    ...
    from app.modules.workspace.services import sandbox_service as module

    monkeypatch.setattr(
        module.workspace_settings,
        "browser_proxy_urls",
        [_FakeSecret("http://a:8080")],
    )
    assert _provisioning_env(SandboxKind.FUNCTION) == {}


def test_a_workspace_sandbox_gets_one_proxy_from_the_pool(monkeypatch) -> None:
    from app.modules.workspace.services import sandbox_service as module

    pool = [_FakeSecret("http://a:8080"), _FakeSecret("http://b:8080")]
    monkeypatch.setattr(module.workspace_settings, "browser_proxy_urls", pool)

    env = _provisioning_env(SandboxKind.WORKSPACE)
    assert env["LEMMA_BROWSER_PROXY_URL"] in {"http://a:8080", "http://b:8080"}


def test_an_empty_pool_assigns_nothing(monkeypatch) -> None:
    from app.modules.workspace.services import sandbox_service as module

    monkeypatch.setattr(module.workspace_settings, "browser_proxy_urls", [])
    assert _provisioning_env(SandboxKind.WORKSPACE) == {}


class _FakeSecret:
    """Just enough of `SecretStr` for `_provisioning_env` to use."""

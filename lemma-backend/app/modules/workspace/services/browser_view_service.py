...
class BrowserViewService:
    ...
    async def open_session(
        ...
        *,
        ...
        found = await relay.ensure_browser(origin=origin, session=session)
        ...
        return await relay.session_socket_url(
            target_id=target_id, mode=mode, session=session
        )

    async def save_login_state(self, user_id: UUID, *, domain: str) -> dict:
        relay = await self._relay(user_id, start=True)
        return await relay.save_state(domain=domain)

    async def load_login_state(
        self, user_id: UUID, state: dict, *, domain: str
    ) -> None:
        relay = await self._relay(user_id, start=True)
    ...
    async def ensure_for_sign_in(self, user_id: UUID, *, origin: str) -> None:
        ...
        """
        relay = await self._relay(user_id, start=True)
        await relay.ensure_browser(origin=origin, session=None)

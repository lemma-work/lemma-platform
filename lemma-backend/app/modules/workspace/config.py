...
Everything about provisioning and reaching a sandbox. Each field keeps its
existing environment variable as an alias, because these names are set by
lemma-stack, locald and the Desktop host pack -- a rename here would silently
stop reading a value those already write.
"""
...
class WorkspaceSettings(BaseSettings):
    ...
    workspace_image: str = Field(
        ...
        validation_alias=AliasChoices(
            "WORKSPACE_IMAGE", "AGENTBOX_WORKSPACE_IMAGE"
        ),
        description="Container image backing workspace sandboxes",
    )
    function_image: str = Field(
        default="agentbox-function:dev",
        validation_alias=AliasChoices("FUNCTION_IMAGE", "AGENTBOX_FUNCTION_IMAGE"),
    ...
    workspace_profile_name: str = Field(
        ...
        validation_alias=AliasChoices(
            "WORKSPACE_PROFILE_NAME", "AGENTBOX_WORKSPACE_PROFILE_NAME"
        ),
    ...
    workspace_profile_digest: str = Field(
        ...
        validation_alias=AliasChoices(
            "WORKSPACE_PROFILE_DIGEST", "AGENTBOX_WORKSPACE_PROFILE_DIGEST"
        ),
    ...
    function_profile_name: str = Field(
        ...
        validation_alias=AliasChoices(
            "FUNCTION_PROFILE_NAME", "AGENTBOX_FUNCTION_PROFILE_NAME"
        ),
    ...
    function_profile_digest: str = Field(
        ...
        pattern=r"^sha256:[0-9a-f]{64}$",
        validation_alias=AliasChoices(
            "FUNCTION_PROFILE_DIGEST", "AGENTBOX_FUNCTION_PROFILE_DIGEST"
        ),
    ...
    runtime_credential_key: Optional[str] = Field(
        default=None,
        validation_alias=AliasChoices(
            "WORKSPACE_RUNTIME_CREDENTIAL_KEY", "AGENTBOX_RUNTIME_CREDENTIAL_KEY"
        ),
    ...
    idle_release_seconds: int = Field(
        default=900,
        validation_alias=AliasChoices(
            "WORKSPACE_IDLE_RELEASE_SECONDS", "AGENTBOX_WORKSPACE_IDLE_SECONDS"
        ),
        description=(
    ...
    docker_socket_path: str = Field(
        ...
        validation_alias=AliasChoices(
            "WORKSPACE_DOCKER_SOCKET_PATH", "AGENTBOX_DOCKER_SOCKET_PATH"
        ),
    ...
    docker_private_network: Optional[str] = Field(
        default=None,
        validation_alias=AliasChoices(
            "WORKSPACE_DOCKER_PRIVATE_NETWORK", "AGENTBOX_DOCKER_PRIVATE_NETWORK"
        ),
    ...
    docker_allow_mutable_images: bool = Field(
        default=False,
        validation_alias=AliasChoices(
            "WORKSPACE_DOCKER_ALLOW_MUTABLE_IMAGES",
            "AGENTBOX_DOCKER_ALLOW_MUTABLE_IMAGES",
        ),
        description=(
    ...
    add_host_gateway: bool = Field(
        default=False,
        validation_alias=AliasChoices(
            "WORKSPACE_ADD_HOST_GATEWAY", "AGENTBOX_ADD_HOST_GATEWAY"
        ),
        description="Map the host gateway into sandboxes under host_alias",
    )
    host_alias: Optional[str] = Field(
        default=None,
        validation_alias=AliasChoices("WORKSPACE_HOST_ALIAS", "AGENTBOX_HOST_ALIAS"),
    ...
    # Desktop writes the AGENTBOX_LOCAL_* names (lemma-stack/config/render.py),
    # so those stay readable here.

...
async def test_a_leaked_tool_call_is_stripped_from_the_delivered_answer():
    """The QA report, driven through the real buffer-and-deliver path.

    Asked for a row count, the model wrote its tool call on the *content*
    channel and the answer after it, and the whole string was delivered:

        {"tool_name":"pod_query","args":{...}}6

    Stripping in `_assistant_text_from_event` is what keeps the envelope out of
    the buffer, so this asserts the delivered message rather than the helper.
    """
    service = _SurfaceService()
    observer = _observer(service)
    conversation = SimpleNamespace(id=uuid4(), metadata={"surface_platform": "TELEGRAM"})
    ...
    await observer.on_event(
        _assistant(
            MessageDraft.of_text(
                '{"tool_name":"pod_query","args":{"sql":"select count(*)"}}6'
            )
        ),
        conversation,
        SimpleNamespace(),
    ...
    assert service.messages == [
        {"conversation_id": conversation.id, "message": "6"}
    ]


async def test_an_answer_that_is_only_a_leaked_tool_call_is_not_silence():
    """Stripping must not turn a bad answer into no answer.

    Before this the user saw the envelope -- wrong, but visible. Delivering an
    empty string instead reads as the agent ignoring them, which is the one
    ...
    await observer.on_event(
        _assistant(MessageDraft.of_text('{"tool_name":"pod_query","args":{}}')),
        conversation,
        SimpleNamespace(),
    )
    await observer.on_run_finished(conversation, SimpleNamespace())
    ...
    assert len(service.messages) == 1
    assert service.messages[0]["message"], "the turn ended in silence"

"""Where an external thread lives as a conversation in a pod.

Split from :mod:`surface_repository`, which held both this and the surface
installations themselves -- two tables, two lifetimes, and one file over the
size the architecture ratchet allows. Nothing here changed in the move.
"""
...
class SurfaceConversationLinkRepository:
    ...
    async def update_last_event(
        ...
        *,
        link_id: UUID,
        # The parsed event as JSON, on its way to a JSONB column. `object`
        # rather than `Any` because nothing here reads into it -- it is stored,
        # and the entity is what gives it a shape again on the way out.

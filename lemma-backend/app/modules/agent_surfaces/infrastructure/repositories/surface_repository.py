...
from app.modules.agent_surfaces.infrastructure.repositories.surface_conflict_sql import (
    account_conflict_in_org,
    platform_identity_holder,
    system_credential_conflict_in_org,
)
...
class SurfaceRepository(SurfaceInstallationRepositoryPort):
        ...
        *,
        ...
        result = await self.session.execute(
            system_credential_conflict_in_org(
                pod_id=pod_id,
                platform=platform,
                exclude_surface_id=exclude_surface_id,
            )
        )
        model = result.scalar_one_or_none()
        return model.to_entity() if model else None
    ...
    async def get_platform_identity_holder(
        ...
        *,
        ...
        """Who already answers as this bot; see `platform_identity_holder`."""
        row = (
        ...
        if row is None:
            return None
        model, same_org = row
        # A retired row holding the identity still holds it, but it cannot be
        # named: `to_entity_or_none` is what keeps an unreadable row from
        # 500ing an unrelated creation (see `get_account_conflict_in_org`).
        surface = model.to_entity_or_none()
        if surface is None:
            return None
        return PlatformIdentityHolder(surface=surface, same_org=bool(same_org))

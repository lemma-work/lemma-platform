...
def test_it_states_the_budget_because_nothing_else_does() -> None:
    ...
    message = _expired_run_error(_run(budget_seconds=600, started_ago=660), now=datetime.now(timezone.utc))
...
def test_it_says_the_sweep_ended_it_not_the_function() -> None:
    message = _expired_run_error(_run(budget_seconds=600, started_ago=700), now=datetime.now(timezone.utc))
...
def test_it_is_distinguishable_from_the_dispatchers_inline_timeout() -> None:
    ...
    swept = _expired_run_error(_run(budget_seconds=120, started_ago=130), now=datetime.now(timezone.utc))
...
def test_a_run_with_no_timestamps_still_gets_a_usable_reason() -> None:
    message = _expired_run_error(
        SimpleNamespace(started_at=None, deadline_at=None), now=datetime.now(timezone.utc)
    )

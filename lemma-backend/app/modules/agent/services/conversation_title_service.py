...
class ConversationTitleGenerator:
    ...
    def __init__(
        ...
        *,
        ...
        # One seam where there were two. Admission and recording are the same
        # operation seen from two ends, and `billed` is that operation: it
        # reserves on the way in, records each request, and settles on the way
        # out. Injecting the pair separately let a test arrange a reservation
        # that nothing would ever settle.
        metering: MeteringScope | None = None,
    ) -> None:
    ...
    async def generate(
        ...
        *,
        ...
        async with (self._metering or billed)(
            model,
            source_type="conversation_title",
            runtime_profile=runtime_profile,
            ...
            agent = (self._llm_agent or PydanticAIAgent)(
                metered_model, system_prompt=_TITLE_SYSTEM_PROMPT
            )
            result = await agent.run(
                _build_user_prompt(user_text, reply_text),
                # Limits read off the *unwrapped* model: `supports_token_precount`
                # unwraps to answer, but asking it about the model the caller
                # actually built keeps the question about the provider rather
                # than about how many layers happen to be around it.

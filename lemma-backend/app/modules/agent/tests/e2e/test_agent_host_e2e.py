...
async def _create_run(
    *,
    ...
    run = await ConversationRepository(uow).create_agent_run(
        ...
        metadata={"source": "agent_host_e2e"},
    )
    await uow.commit()
...
async def test_agent_host_is_durable_fenced_and_idempotent(
    ...
    harness_id = UUID(publish.json()["items"][0]["id"])
    ...
    profile_response = await authenticated_client.post(
        ...
        json={
            ...
            "harness_snapshot_revision": "codex-config-revision-1",
    ...
    # Harness catalogs are live, not copied into profiles. A new snapshot is
    # visible immediately, while execution stays fenced until explicit refresh.
    ...
    assert live_profile["availability_status"] == "CONFIG_REVISION_MISMATCH"
    assert live_profile["harness_config_revision"] == "codex-config-revision-2"
    assert {item["name"] for item in live_profile["model_catalog"]} >= {
        "gpt-test",
        "gpt-future",
    }
    profile_refresh = await authenticated_client.post(
        f"/organizations/{fixed_test_org['id']}/runtime/profiles/{profile_id}/refresh"
    )
    assert profile_refresh.status_code == 200, profile_refresh.text
    assert profile_refresh.json()["availability_status"] == "READY"
            ...
            "rejections": [
                {
                    ...
                    "code": "CAPACITY_LOST",
                    "retryable": True,
                    "detail": "capacity changed before dispatch",
                }
            ],
        },
    )
    assert rejected.status_code == 200, rejected.text
    assert rejected.json()["commands"] == []
    await db_session.refresh(
        await db_session.get(AgentHostRunLeaseModel, run_id)
    )
    redelivered = await authenticated_client.post(
        "/agent-host/poll",
        headers=device_headers,
        json={
            "hello": hello,
    ...
    assert [item["command_id"] for item in redelivered.json()["commands"]] == [
        command["command_id"]
    ]
    ...
    # A duplicate/reordered rejection arriving after durable acceptance is an
    # idempotent no-op and cannot roll the run back to QUEUED_FOR_HOST.
    stale_rejection = await authenticated_client.post(
        "/agent-host/poll",
        headers=device_headers,
        json={
            "hello": hello,

...
async def test_ensure_github_credentials_skips_when_already_provisioned(
    ...
    async def _fail_resolve(ctx):
        raise AssertionError("must not resolve a token when already provisioned")

    monkeypatch.setattr(bridge, "_resolve_github_access_token", _fail_resolve)
...
async def test_ensure_github_credentials_caches_no_account_as_unavailable(
    ...
    monkeypatch.setattr(bridge, "_resolve_github_access_token", _no_account)
...
async def test_ensure_github_credentials_writes_credential_file_and_marks_provisioned(
    ...
    monkeypatch.setattr(bridge, "_resolve_github_access_token", _token)
    ...
    assert "credential.helper" in session.commands[0]
    assert "/tmp/.git-credentials" in session.commands[0]
    ...
    assert "gho_faketoken123" not in session.commands[0]

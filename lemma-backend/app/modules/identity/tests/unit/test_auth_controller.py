...
def _signed_resend_request(body: bytes, secret: str):
...
async def test_resend_bounce_deactivates_only_permanent_failures(
    ...
    monkeypatch.setattr(
        auth_controller, "_deactivate_email_for_hard_bounce", deactivate
    )
    body = json.dumps(
        {
            "type": "email.bounced",
    ...
    response = await auth_controller.accept_resend_email_bounce(

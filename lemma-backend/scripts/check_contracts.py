...
def render(module: str, rows: list[tuple[str, str, str, str]]) -> str:
    lines = [
        ...
        "[the product specification](../../../docs/product/README.md). This says "
        "what each operation does; that says what any of it is for.",
        "",
        "The table below is generated from the committed OpenAPI specification by "
        "`scripts/check_contracts.py --write`. Add the behaviour in prose under "
        "each operation's heading, outside the generated block — that part is "
...
def render_events(analytics: set[str]) -> str:
    ...
    lines = [
        ...
        "[`app/core/analytics/event_catalog.py`](../../app/core/analytics/event_catalog.py), "
        "which is append-only and already gated: emitting a name absent from it "
        "raises in development and CI. This document is where the *meaning* "
        "lives — when it fires, exactly once or at least once, and what a "
        "consumer may assume.",
        "",

...
| SCHEDULE-001 | **Resolved** | Durable schedule-run ledger keyed by schedule/source event, stable source IDs, failed/retryable filter outcomes, outbox target publication, run listing, and durable consecutive-failure state. Redis fire-store code was removed. Tables/column are in revision `0003`. | Full schedule E2E including schedule-run deduplication and automatic delivery recovery; `test_schedule_processor.py`; `test_schedule_idempotency_regression.py`; `test_reliability_adapters.py`. |

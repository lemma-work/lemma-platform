...
def test_watching_does_not_take_the_wheel_off_the_agent(monkeypatch, tmp_path) -> None:
    """An open panel must cost the agent nothing.

    The lease used to be taken the instant a control socket opened. For a
    sign-in that is harmless -- the session is `login-<host>`, which the agent
    never touches -- but an ordinary watch attaches to the agent's *own*
    session, so simply having the panel on screen stopped the agent browsing.
    The fix that shipped first was to open the pane read-only, which traded
    that for a browser nobody could click.

    So the lease is taken by use. Nothing here has clicked yet.
    """
    from sandbox_runtime.browser_relay import app as relay_app

    monkeypatch.setattr(relay_app, "_WHEEL_DIR", tmp_path / "wheel")
    wheel = relay_app._WheelOnUse("conv-abc")

    assert not relay_app.wheel_path("conv-abc").exists()

    wheel.touch()
    assert relay_app.wheel_path("conv-abc").exists(), "a click takes the wheel"

    wheel.let_go()
    assert not relay_app.wheel_path("conv-abc").exists()


def test_the_wheel_comes_back_once_the_person_stops(monkeypatch, tmp_path) -> None:
    """Held while somebody is working, released when they walk away.

    Without the release an agent would be blocked for as long as the panel
    stayed open after a single stray click, which is the same failure as
    taking the lease on connect, only harder to notice.
    """
    from sandbox_runtime.browser_relay import app as relay_app

    monkeypatch.setattr(relay_app, "_WHEEL_DIR", tmp_path / "wheel")
    monkeypatch.setattr(relay_app, "_WHEEL_IDLE_SECONDS", 0.0)
    wheel = relay_app._WheelOnUse("conv-abc")

    wheel.touch()
    assert relay_app.wheel_path("conv-abc").exists()

    # What `release_when_idle` does each tick, without waiting on its sleep.
    wheel._last_input -= 1.0
    if time.monotonic() - wheel._last_input >= relay_app._WHEEL_IDLE_SECONDS:
        wheel.let_go()

    assert not relay_app.wheel_path("conv-abc").exists()


def test_moving_the_mouse_over_the_page_is_not_driving() -> None:
    """Reading is not driving.

    noVNC emits a `PointerEvent` for every mouse move across the canvas. If
    those counted, resting the cursor over the pane while reading would take
    the wheel off the agent for a minute at a time -- the exact thing holding
    the lease by use is meant to avoid. A zero button-mask is the cursor
    passing through; anything else is a click or a drag.
    """
    from sandbox_runtime.browser_relay.stream_proxy import _carries_real_input

    motion = bytes([5, 0, 0, 10, 0, 20])
    click = bytes([5, 1, 0, 10, 0, 20])
    keypress = bytes([4, 1, 0, 0, 0, 0, 0, 65])
    asking_for_pixels = bytes([3, 1, 0, 0, 0, 0, 0, 10, 0, 10])

    assert _carries_real_input(motion) is False
    assert _carries_real_input(asking_for_pixels) is False
    assert _carries_real_input(click) is True
    assert _carries_real_input(keypress) is True

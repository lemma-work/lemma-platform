...
| `import_pod_github` | `pod-import-plan:{id}` | resolve GitHub account (short UoW) → fetch zipball (no DB) → stage → fall through to plan routine |
...
| `publish_pod_github` | `pod-publish:{id}` | resolve account + snapshot (short UoW) → build bundle + README (+ optional AI polish; degrades, never fails) (no DB) → GitHub: create repo, then one atomic git-data commit (blobs → tree → commit → ref) → `completed` with `repo_url` |
...
- **No direct GitHub API client.** All GitHub I/O goes through Lemma's own GitHub connector (same consent/account model as every other connector) — the same native account a workspace agent's `git`/`gh` use, not a brokered second identity.

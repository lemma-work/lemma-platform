...
1. Confirm `*.lemma.localhost` credentialed CORS, redirects, service workers, WebSockets, and SSE in WKWebView, WebView2, Safari, Chrome, Firefox, and provider OAuth consoles. The macOS matrix proved that WKWebView discards the session from a different API hostname and rejects a parent-domain `.lemma.localhost` cookie. The main UI and API therefore use the same `app.lemma.localhost` hostname on separate ports with a host-only cookie. Production-parity built-app origins require a one-time per-origin session bootstrap and remain release-blocked until that flow and the remaining supported-client matrix pass.

...
- **agent host**: `require_agent_host_caller` — control, the splash, or the
  workspace on the origin this app navigated to (hosted or local). While
  sharing is on that origin *is* the shared one, so this check does not refuse
  it; the capability, which never lists a shared origin, does.
